...
『起一下 phone-bridge』」——他照着在电脑上跟那边的 agent 说一句话，那边读到这一节就
...
听到「起一下 phone-bridge」时，先看一眼这台机器上有没有前端在跑：
...
- **状态点**：列表里还在跑的会话有个点。**绿 = 在跑且闲着**；**橙色闪 = 在跑且
  ...
  还在，只能看不能发。
- **能不能发，只有两类，而且只在"在跑的会话"上标**：在跑且起了中继 → 绿色
  「可发送」；在跑但没起中继 → 橙色「待启动」**整行变灰**。没在跑的不标——历史
  对话本来就不能发，codex 那 80 条更是每条都不能写，逐条挂一个「只读」只是噪音。
- **灰的那一类，点进去要告诉用户怎么办**：「在那个对话里跟它说一句『起一下
