...
```
手机 Safari
  │  Tailscale 内网（没装就退到局域网，见「绑定地址」）
  ▼
bridge.py  ← HTTP 服务，默认绑 127.0.0.1 + Tailscale IP（没有就用局域网 IP）
...
- **读**：`/api/sessions` 一次列出**所有** App 的会话（claude 和 codex 混在同一个
  列表里），`/api/messages?app=&sid=` 挑其中一个。手机端就一个 URL、一个页面，
  翻的就是全部对话。**每个 App 只出最近 10 条**（`bridge.py` 的 `PER_APP_LIMIT`）
  ——Codex 盘上有几百个线程，全推过去一屏刷不到头，而手机上要找的永远是"最近
  在说的那几件事"。想看更旧的，去电脑上。
- **写**：两条路，按「离那个对话有多近」排：
...
### Codex 的入站：`codex queue`，不用中继

Codex 桌面版是 `/Applications/ChatGPT.app`，跑着一个 `codex app-server`
（stdio 传输，由 Electron 主进程驱动）。入站就一条命令：

```bash
codex queue --thread <线程 id> --message "正文"
```

实测够了：消息落成**一条普通用户消息**（rollout 里
`content_item_kinds: ["user.text"]`，跟手打的逐字段一致，四条并排看不出哪条是
注入的），app-server 当场跑完整一轮，回答再从日志读回来。**窗口开没开都一样
能发**——关着也会跑，回头打开窗口就看到新一轮。所以 Codex 这边**不需要中继**，
也不吃进程树那一套。

几个逆出来的结论，别再走弯路：

- `~/.codex/ipc/ipc.sock` 归 ChatGPT.app 主进程，**认对端身份**：python 连上去
  **发任何字节都秒关**（0.0 秒 EOF，发半个 JSON 也一样），什么都不发反而连得住；
  只有 App 自带那份签过名的 `codex` 二进制能进。**所以别自己拼协议，shell 出去
  调它。**
- **要用 App 自带那份**：`/Applications/ChatGPT.app/Contents/Resources/codex`
  （0.154.0-alpha）。PATH 上是 0.151.0，跟正在跑的 app-server 不是同一个版本。
- 官方远程控制（`codex app-server daemon enable-remote-control`）**没用上**——
  它会改 Codex 的持久配置，而且本机没有 managed daemon
  （`app-server-control.sock` 不存在）。`queue` 是现成且不改配置的那条路。
- 失败**不会静默**：不存在的线程 id 会带回真正的 JSON-RPC 错误
  （`no rollout found for thread id …`）。

**读的一侧有两个坑（都已修）：**

1. 行 id 用 `session_meta.payload.id`，**不是 `session_id`**。子代理（guardian）
   也各写自己的 rollout，它们的 `session_id` 指向父会话、`id` 才是自己——拿
   `session_id` 当行 id，一个会话挂的三十个子代理会全塌成一行（80 行只剩 37 个
   唯一 id、还互相顶掉）。子代理不进列表：它们是内部机器，不是用户开的对话。
2. 一个线程**横跨多个 rollout 文件**：resume / 重开窗口会新起一个分段文件，
   里面只有当时那一段窗口。最极端的一个线程有 13 段，而**最新那份只有 1 条
...
**起完必须把地址和二维码交到用户手上。** 用户要的是"手机一扫就进去"，不是一段日志
——而且他根本看不到日志（服务是以后台任务起的，输出落在任务输出文件里）。所以：

1. 从启动横幅里取**远端那个**地址（`127.0.0.1` 是电脑自己连自己用的，别给）。
2. 把地址**写在回复正文里**。
3. 把那张图**内嵌进回复**——读 `QR_PNG_PATH`、base64 一下、写成
   `![手机扫码 <地址>](data:image/png;base64,…)`。**实测渲染得出来**，用户直接
   在对话里看到就能扫，不用点附件、不用开侧栏。

横幅上写的是哪个**就转哪个，别自己拼一个**：

- 写 **Tailscale IP** → 正常情况，直接发。
- 写 **局域网地址** → 发，但**必须带一句**"这是局域网地址，同一个 wifi 下的设备都能
  打开，出门在外别用"。横幅上已经警告过一次，用户看不到横幅，得靠你转达。
- 写 **"没找到任何远端地址"** → **照实说**，让他去装 Tailscale
  （<https://tailscale.com/download>）再重启。**这时候没有二维码**（给回环地址画
  一张，手机扫了打不开，比不给更坏），别硬凑一个地址糊过去，也别拿回环地址当远端发。

几条弯路别再走：

- **别用文字/符号在回复里"画"二维码**（`██` 拼一个）。聊天的等宽字体高宽比不保证，
  画出来很可能扫不动。内嵌的是一张标准黑白位图，实测能被 macOS 自己的识码器解出来。
- **别只丢一个附件**（`SendUserFile`）。它在大对话里长成**一张卡片**（文件名 + 大小），
  不摊开，用户会以为是没渲染出来。要它出现在侧栏还得先复制进会话的工作目录
  （`/tmp` 侧栏够不着），白留一个垃圾文件。内嵌那条路没这些问题。

服务器启动时自己就存好了那张图（`_print_qr` 里的 `QR_PNG_PATH`）。`qrcode` 没装的话
它会**当场 `pip install --user` 装一个**（会先打印一行说自己在装），所以正常情况下
...
启动时按顺序挑，**一定绑到一个远端地址**，不会静默退回本机：

| 顺序 | 挑什么 | 手机连得上吗 |
|---|---|---|
| 1 | `tailscale ip -4` 查到的 IP | 能，同一 tailnet 内 |
| 2 | 本机在局域网里的 IPv4 | 能，但**同一网络里谁都够得着** |
| 3 | 两个都没有 | **不能**。只绑回环，横幅会明说要装 Tailscale |
...
`--host <ip>` 自己指定绑哪儿；`--no-tailscale` 只绑回环（Tailscale 和局域网都不绑）。

**别绑 `0.0.0.0`。** 那会连咖啡馆的 wifi 一起暴露——能碰到这个端口的人就能往你的
AI 会话里发消息。

| 办法 | 说明 |
|---|---|
| 连横幅上打印的地址 | **默认就是这样**：Tailscale 优先，没有就局域网 |
...
- **状态点**：列表里还在跑的会话有个点。**绿 = 在跑且闲着**；**橙色闪 = 在跑且
  ...
  还在，只能看不能发。
- **列表分两组**：「已启动 phone-bridge」（`can_send`）在上、「未启动 phone-bridge」
  在下，两个小标题把列表切开。标题写的是**状态**（服务起没起），不是"能不能聊"
  ——前者是事实，后者要用户自己推。顺序还是各组内按最后活动时间。
  「已启动」是 0 的时候要有一句话——那正是用户最需要提示的时刻（见下一条）。
  注意对 **codex** 来说这一组的含义是"**有可用的写入通道**"，不是字面上的"起了
  服务"——codex 不发中继，桌面版开的会话天然就能写。
- **两组之间是赞助位**（见「赞助位」一节）。**但第一组够 5 条时它会插到第 5 条
  下面**——第一组动不动五六十条（Codex 一口气能上来几十个），卡在两组之间等于把
  广告推到几屏之外。不够 5 条就还是留在组间。两种情况都实测过。每个赞助商三行：**名字**（链接）、
  一句详情、**网址**（同一链接，小灰字）。名字和网址都要，因为只给名字的话，
  用户看不出点进去的是谁家域名；没有 `url` 的那条就是两行纯文字，不是链接。
  它长得刻意不像会话：没有状态点、没有左右推挤的行，只有一条浅底和一个小小的
  「赞助」标签，别让人以为点进去能聊天。
- **只有一种标签：「待启动」**（橙色，整行变灰）。它标的是"其实在跑，只是还没启动
  服务"，所以**点进去有救**。其余一概不标——分组标题已经说了能不能聊，历史对话和
  终端里跑的 codex 那种是"本来就不能写"，逐条挂一个「只读」只是噪音。
...
### 赞助位：内容在 GitHub 上，不在代码里

列表里那条广告（**位置**：第一组够 5 条就插在第 5 条下面，不够就卡在两组之间）。
**要换广告，改仓库根目录的 `ads.json` 就行**——不用碰
这台机器、不用重启、不用改代码：

```json
{ "ads": [ { "name": "名字（有 url 时它才是链接本身）", "desc": "一句话", "url": "https://…" } ] }
```

`https://github.com/bang919/phone-bridge` → `ads.json` → 右上角铅笔改 → commit。
默认最长 5 分钟后手机上就变了。**只填 `name` 和 `desc` 也行**（比如"广告位招租，
微信 xxx"）——没有 `url` 就画成纯文字，不做成链接。

- 服务端（`bridge.py` 的 `load_ads()`）每 5 分钟去 `raw.githubusercontent.com`
  拉一次，缓存住；手机只跟 bridge 说话，不直连 GitHub。
- **拉到了就一律用网上的那份，本地那份只是兜底。** 改完 `ads.json` 得先
  `git push` 才会生效——改本地文件不算数。
- **拉不到就用技能目录里自带的 `ads.json`，再不行就整块不显示。** 没网、被墙、
  json 写坏了，页面照常能用——广告永远不该把功能弄挂。
- 换源：`PHONE_BRIDGE_ADS_URL=<任意 URL>` 起 bridge。想彻底不要广告，
  把 `ads.json` 改成 `{"ads": []}`。
- **`url` 只收 `http(s)`，两端都校验**（服务端 `_clean_ads()` 挡 `javascript:`，
  手机端 `escAttr()` 转引号防属性突围）。这份内容来自网络，别把它当自己人。
  `url` 写坏了**只丢 url、不丢整条**——内容还在，总比整条消失强。
